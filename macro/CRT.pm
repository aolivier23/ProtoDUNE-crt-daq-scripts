...
checkROPLog sendmail check_last_disk_used loadpmtdata_auto loadpmtdata baselinemb initializeusb initializefilestructure takedatamb generatecsv2 plotdatamb2 setpmtdata getpmtdata
...
$OVSERV_HOST $OVSERV_IP $mysql_table
$runcontrol_hostname $usedogsifier $HWthreshold $eb_flag
...
require CRTfunctions;

our ($client_sock,$client_sock_OVRC,$DebugDAC,$pmt,$usb,$pmtserialnumber,$HVsetting,$lasersinglepe,$mysqlload,
$DACt,$thresh,$hdelay,$runlength,$newrunlength,$pipedelay,$force_trig,$meanpulseheight,$gate,$gateonoff,$elapsed_time,$mysql_table,
...
our $onlinetable = "online_normal_running"; # this is the default table for the timing test# "online800";
...
our ($pmt_local, $usb_local, $usb_local1, $totalpmt, @pmttousb, @pmttoboard, @structure);
our ($totalbox, @boxtousb, @boxtoboard);
our ($oldsec,$oldmin,$oldhour,$oldday,$oldmonth,$oldyr19,@oldrest);

my $mysqlSocket = "/var/lib/mysql/mysql.sock"; #TODO: Detect whether to use this socket based on whether artdaq is set up?
...
eval {
	$dsn = "dbi:mysql:database=$database;host=$DBhostname;mysql_socket=$mysqlSocket"; 
...
#	print "\nLoading settings from MySQL table: $onlinetable\n\n";
	...
	if(!$triggerbox) {
	    ...
	    elsif($pmt_board eq 3) { $pmtdata[$usb_board][$pmt_board][3]=932; } #DAC
            elsif($pmt_board eq 14) { $pmtdata[$usb_board][$pmt_board][3]=932; } #DAC
	    ...
	    $usbhowmanyboards{$usb_board}++;
	}
	else {
	    $pmtdata[$usb_board][$pmt_board][0]='none'; # pmt serial number
	    $pmtdata[$usb_board][$pmt_board][1]= $pmt; #board number
	    $pmtdata[$usb_board][$pmt_board][2]=-999; # HV
	    $pmtdata[$usb_board][$pmt_board][3]= 932; # DAC threshold
	    $pmtdata[$usb_board][$pmt_board][4]="no"; # use m2 gains
	    $pmtdata[$usb_board][$pmt_board][5]="off"; # gate (on, off,..)
            if($pmt_board <=40) {
	        $pmtdata[$usb_board][$pmt_board][6]=20; # pipedelay
...
print("DataPath=$DataPath,Disk=$disk,DataFolder=$DataFolder\n");
...
$run = $run_number;
print "pmtini = $pmtini, pmtfin = $pmtfin\n";
...
#        print "usb_local=$usb_local pmt_local=$pmt_local\n";
	...
	com_usb $usb_local, $pmt_local, 75, 0b00010000;     # trigger mode
	com_usb $usb_local, $pmt_local, 86, 0;              # edge strip mode is off
	com_usb $usb_local, $pmt_local, 87, 0b00;           # force readout -> 01: 1msec, 10: 16msec, 11: 256msec
...
##	tarry 0.01;
	...
	com_usb $usb_local, $pmt_local, 254, 1;                      # disable trigger 
	...
	if ($pmt_local==7 || $pmt_local==15 || $pmt_local==27 || $pmt_local==31) {
	    for(my $i=1; $i<=100; $i++) {               # removed the +400 CM 10012018
	        com_usb $usb_local, $pmt_local, 81, 0;
	    }
	}
	com_usb $usb_local, $pmt_local, 255, 0;                      # disable trigger 
   ...
   print "\n Waiting for late packets ";
    ...
    for(my $e=0; $e<=10; $e++){    ###wait for late packets
           tarry 0.1;
           print "*";
    }
...
###now let's initialize everything for the data taking
	...
	com_usb $usb_local, $pmt_local, 73, $gateonoff;        # gate alladc gate on -> 0b11110
	#      alladc gate off-> 0b01110
	com_usb $usb_local, $pmt_local, 85, $pipedelay;         # set up pipe delay
...
sub test_system_time {
    ...
    my $disk = "1";
    my $DataPath = "CRTTime/DATA/";
    my $DataFolder = "/data${disk}/${DataPath}";

#$DataPath = "CRTTime/DATA";
#$disk = 1;  # AJF: This is also set in startOVDAQ.pl.  The setting here makes that one moot.
...
our ${Time_Data_Path} = "${DataFolder}/Run_${time_run_number}/binary";

print "Initializing time check for the CRT .";
...
for (my $pmt1 = 1; $pmt1<=$totalpmt; $pmt1++) {
    ...
    com_usb $usb_local, $pmt_local, 80, 5;              # hold delay is variable. Has been fixed to 5 here	
    dac_usb $usb_local, $pmt_local, 1000;               # threshold value
    com_usb $usb_local, $pmt_local, 73, 0b01110;        # gate alladc gate on -> 0b11110
                                                        #      alladc gate off-> 0b01110
    com_usb $usb_local, $pmt_local, 85, 5; #$pipedelay;         # set up pipe delay
...
#############
    ...
    $full_path = "${DataFolder}Run_${run_number}/binary";

    print "$full_path \n";

    my $tempfile;
    my @files = <${full_path}/*.wr>; #exclude baselines
                                       
    foreach $tempfile (@files){
	if($tempfile =~ /1(\d+)_(\d+).wr$/) {
	    print "$tempfile\n $1,$2\n";
	    system qq|mv "$tempfile" "${full_path}/1$1_$2"|;
	}
    }
}
...
my $dsn = "dbi:mysql:database=$HVdatabase;host=$DBhostname;mysql_socket=$mysqlSocket";
...
sub check_rate {
   ...
   print "Getting baselines from files @files.\n";
	...
	foreach $tempfile (@files) {
	    ...
	    open IN, "/usr/bin/perl -S decode.pl \"${tempfile}\" |";
	    while(<IN>) {
		chomp ;
		next if /^\s*$/;
		my @line = split /\s*,\s*/;
...
#		sendmail("OVRC","CRITICAL","PMT Board $pmttoboard[$i] is missing from the baseline file");
    ...
    foreach $key (sort (keys(%hit_pmt)))
    {
      print "PMT: $key Hits: $hit_pmt{$key}\n";
    }
...
#	      gaibu_msg($MNOTICE, "PMT $key missing fraction of triggers: $tot_missed/$hit_pmt{$key}");
	...
	print "Error minimum number of baseline packets found is $keymin for board $pmtboard_keymin\n"; 
...
$Data_Path = "/data$disk/CRTDAQ/DATA/Run_$run/decoded/";

tarry 5;

$readfile = 0;

if (-e "/data$disk/CRTDAQ/DATA/Run_$run/decoded" ) {
...
foreach my $file (@files) {
	...
	while(<IN>) {
	    ...
	    if($_ =~ /Data Folder.*CRTDAQ\/DATA/ and $FoundRun == 1) { $FoundDataFolder = 1; }
...
if($TotErr > $TotErrThresh) {
    print "Warning! Found $TotErr new errors from CRTDAQ\n";
    gaibu_msg($MERROR,"ERROR! Found $TotErr new errors from CRTDAQ\n");
    sendmail("OVRC","CRITICAL","Found $TotErr new errors from CRTDAQ");
}
...
my $dsn = "dbi:mysql:database=$LVdatabase;host=$DBhostname;mysql_socket=$mysqlSocket";
...
gain47, gain48, gain49, gain50, gain51, gain52, 
gain53, gain54, gain55, gain56, gain57, gain58, 
gain59, gain60, gain61, gain62, gain63, gain64 FROM $mysql_table");
...
$full_path = "/data1/CRTDAQ/DATA/Run_${run_number}/binary";     
...
for(my $i=0;$i<$newrunlength;$i++){
	...
	$filenametemp = "$full_path"."/signal_$i.bin.part";
	...
	tarry 0.75;
     	...
     	system qq|mv "${filenametemp}" "${full_path}/signal_${i}.bin"|;
}
#just at the end
...
# print "\n";
$full_path = "/data1/CRTDAQ/DATA/Run_${run}";

my $full_path_temp = "$full_path";
my $homedir = "/nfs/home/np04daq/crt/readout_linux";

dir $full_path_temp;                              # set usb to (create and) write to this folder

$baselines = "$full_path/binary/baselines.dat";
$filename = "$full_path/binary/signal";
...
#}
my $tempfile;
my @files = <${full_path}/binary/1*_${usb}*>; #exclude baselines                                       
foreach $tempfile (@files) {
    system qq|/usr/bin/perl -S decode.pl "$tempfile" >> "${filename}.dec"|;
}

my $temp1 = "$filename.dec";
#my $temp = "$filename.dec";
#my $temp1 = "$full_path/binary/signal.dec";
#open IN, ">$temp1" or die $!;
#close IN;
#system qq|mv "$temp" "$temp1"|;

#DCfunctions::signal (${full_path},$temp1,$baselines,$pmt);
signal2 (${full_path},$temp1,$baselines,$pmt,${homedir});
...
$full_path = "/data1/CRTDAQ/DATA/Run_${run_number}";
...
system qq|/usr/bin/perl histogram.pl "$full_path" "summary.csv" 9 "Normalized Hits by channel" >> log_${pmtserialnumber}_${date}.txt|;
}
...
#code for retrieving info form mysql database                                                                                      
	...
	eval {
        ...
        gain47, gain48, gain49, gain50, gain51, gain52,                                                                            
        gain53, gain54, gain55, gain56, gain57, gain58,                                                                            
        gain59, gain60, gain61, gain62, gain63, gain64 FROM online");
		...
		if($ref->[1] eq $pmtserialnumber){ #check for matching pmt_serial number and board_address                 
		    ...
		    $foundcount = 1;
		    $mysqlload = 1;
		    #$totalpmt++; ###camillo adde this on 07/04
                    print "Press enter if you want to take data with that settings. If instead you would like to change any settings type change.\n\n";
		    chomp($choicefirst = <STDIN>);
		}
		if($ref->[1] eq $pmtserialnumber){
...
#get "typical module" data                                                                   
    ...
    if(defined($homedir)) {
	open IN, "${homedir}/Scripts_mb/normalized.txt" or goto nohomedir;
	while(<IN>) {
	    chomp;
	    next if /^\s*$/;
                next if /^#/;
...
#removed from above the section gain  - not useful
