...
CRT::load_db_config($db_config_file);
...
#TODO: The next few lines of code only generate signal.dec if it doesn't already exist.  When signal.dec is generated, it only uses the current usb value.  I think 
#      this is resulting in a signal.dec file that only includes "packets" from the first usb number used.  Any subsequent usb numbers used never get their packets put into signal.dec.  
...
$full_path = "/e/h.0/localdev/readout/data1/CRTDAQ/DATA/Run_${run_number}";
